...
- `src/CbUpdate/ClientApp/` - Web Application folder
...
The service worker initialization code is disabled by default. To enable it, uncomment the following code in `src/CbUpdate/ClientApp/src/app/app.config.ts`:
...
Edit [src/CbUpdate/ClientApp/src/app/app.config.ts](src/CbUpdate/ClientApp/src/app/app.config.ts) file:

```
import 'leaflet/dist/leaflet.js';
```

Edit [src/CbUpdate/ClientApp/src/content/scss/vendor.scss](src/CbUpdate/ClientApp/src/content/scss/vendor.scss) file:
...
```
create src/CbUpdate/ClientApp/src/app/my-component/my-component.component.html
create src/CbUpdate/ClientApp/src/app/my-component/my-component.component.ts
update src/CbUpdate/ClientApp/src/app/app.config.ts
```
...
auto-refreshes when files change on your hard drive.
    ...
    npm --prefix ./src/CbUpdate/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/CbUpdate/ClientApp run` command will list all of the scripts available to run for this project.

## Building for production

### .Net Production builds

To build the artifacts and optimize the CbUpdate application for production, run:

```
cd ./src/CbUpdate
rm -rf ./src/CbUpdate/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./CbUpdate.csproj
```

The `./src/CbUpdate/app/out` directory will contain your application dll and its depedencies.

## Testing

### Client tests

Unit tests are run by [Jest][]. They're located in [src/CbUpdate/ClientApp/test/](src/CbUpdate/ClientApp/test/) and can be run with:

```
npm test
```

UI end-to-end tests are powered by [Cypress][]. They're located in [src/CbUpdate/ClientApp/test/cypress](src/CbUpdate/ClientApp/test/cypress)
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"CbUpdate" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t cbupdate .
```

Then run:

```bash
docker run -p 8080:8080 cbupdate
```
